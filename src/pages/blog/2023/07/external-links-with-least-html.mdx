...
export function ExternalLink({ children, rel, ...props }: Props) {
	// retain only lower case unique values
	const rel = [...new Set(`noopener noreferrer ${rel || ''}`.toLowerCase().trim().split(/\s+/))].join(' ')
...
Finally there is also the part of "why?" in the code. You need good comments and/or tests in place to make sure the
solution is not being refactored by anyone too eagerly! "This could be simpler and cleaner", "that is unnecessary"...
those thoughts are so easy to get when seeing something which isn't immediately obvious.
...
```css
/* debug phase: try to have a bog on all links */
a::after {
...
I hope you like this take where I've &ndash; yet again &ndash; solved a modern issue using solutions that are mostly
from the past ages of the web!
